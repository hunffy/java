 ...
 * 	자손 클래스의 객체는 부모클래스의 참조변수로 참조할 수 있다.(자손객체는 부모객체를 가지고있음)
 * 
 * -참조변수와 객체의 자료형이 틀린 경우는 ClassCastException 발생함.
 * 	instanceof 연산자를 이용하여 미리 객체와 참조변수의 관계를 확인할 수 있다.
 */
class Bike3 {
	int wheel;
	int price=10;
...
//		Bike3 b2 = new Bike3(2);	// java.lang.ClassCastException 오류발생
		Bike3 b2 = new AutoBike3(2); //java.lang.ClassCastException 오류발생안함.
		//	형변환이 가능.
		//	부모클래스의 자료형, 자손클래스의 자료형간의 형변환 가능.
		// 자손클래스 <= 부모클래스 형변환시 형변환 연산자 생략 불가
		AutoBike3 ab2 = (AutoBike3)b2; // 오류발생부분
