 ...
 * 	int read(byte[] bur,int start,int len)
...
public class FileInputStreamEx1 {
	public static void main(String[] args) throws IOException {
		...
		//fis.available() : 읽기 가능 한 바이트수.
		byte[] buf = new byte[fis.available()];
		while((data=fis.read(buf)) != -1){
			//String <= byte[]
			//new String(buf,0,data)
			// buf의 내용을 0번 인덱스부터 data길이 만큼 문자열로 생성.
			System.out.print(new String(buf,0,data));
		}
		System.out.println();
		System.out.println("3. read(byte{} buf , int , start , int len) 메서드로 읽기:");
		fis = new FileInputStream(src);
		/*
		 * buf : 0번 인덱스부터 fis.available() 갯수만큼  데이터 저장. 
		 * data : 실제로 읽은 바이트
		 * 
		 */
